...
import { Globe, Lock, Upload, Search, Share2, Trash2, FileText, Users, Key, Database, Shield, Eye, X } from 'lucide-react';
...
export const PublicRAGManager: React.FC = () => {
  ...
  const handleViewDocument = async (docId: string, title: string, author?: string, isPublic?: boolean) => {
    setIsLoadingDoc(true);
    try {
      const client = getPublicRAGClient();
      const { content } = await client.downloadDocument(docId);
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <StatsCard icon={<Database className="w-4 h-4" />} label="Storage" value={stats?.storageUsed ?? '...'} subvalue="Across all your documents" />
        <StatsCard icon={<Shield className="w-4 h-4" />} label="Documents" value={stats?.totalDocuments?.toString() ?? '0'} subvalue={`${stats?.publicDocuments ?? 0} public`} />
              ...
              {documents.map((doc) => (
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="flex gap-2">
                      <button 
                        onClick={() => handleViewDocument(doc.id, doc.title || 'Untitled')}
                        className="p-1.5 hover:bg-[#3B82F6]/10 rounded border border-[#262626] transition-colors"
                        title="View document"
                      >
                        <Eye className="w-3 h-3 text-[#A1A1A1]" />
                      </button>
                      <button 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {communityDocuments.map((doc) => (
              ...
              >
                ...
                <div className="flex gap-2">
                  <button 
                    onClick={() => handleViewDocument(doc.id, doc.title || 'Unnamed', doc.author, true)}
                    className="flex-1 bg-transparent border border-[#262626] text-white py-2 rounded text-[10px] font-bold uppercase tracking-widest hover:bg-white/5 transition-all flex items-center justify-center gap-1"
                  >
        ...
        {viewingDoc && (
          ...
          >
            <motion.div
              ...
              className="bg-[#111111] border border-[#333333] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col"
            ...
            >
                ...
                <div>
                  ...
                  {viewingDoc.author && (
                    <p className="text-xs text-[#717171] uppercase tracking-widest mt-1">
                      By {viewingDoc.author}
                    </p>
                  )}
                </div>
                <div className="flex items-center gap-2">
                  {viewingDoc.isPublic && (
                    <Badge variant="outline" className="text-[8px] uppercase tracking-widest border-[#3B82F6] text-[#3B82F6]">
                      Public
                    </Badge>
                  )}
                  <button
              ...
              <div className="flex-1 overflow-y-auto p-6">
                {isLoadingDoc ? (
                  ...
                  <div className="prose prose-invert max-w-none">
                    <pre className="whitespace-pre-wrap font-mono text-sm text-[#EDEDED] bg-[#0A0A0B] p-4 rounded-lg border border-[#262626]">
                      {viewingDoc.content}
                    </pre>
                  </div>
                )}
              ...
              <div className="px-6 py-4 border-t border-[#333333] bg-[#1a1a1a] flex justify-between items-center">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-white text-black font-bold text-xs uppercase tracking-widest rounded-md hover:bg-white/90 transition-all"
                >
                  Copy Content
                </button>
              </div>
