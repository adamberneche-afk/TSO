...
## 2026-09-21 — floor check 10 adopted: scripts/secrets-doctor.mjs

Added `scripts/secrets-doctor.mjs` (byte-identical to the Mothership hub's canonical copy — do not edit locally, changes belong upstream), `.github/workflows/secrets-doctor.yml`, and `.github/floor.json`.

It confirms every secret this repo's workflows reference is configured and non-empty, with the expected list **derived from the workflow files** rather than written down. That distinction is why it was added here rather than skipped: `tools/doctor/check.js` hand-lists `VERCEL_URL`, `VERCEL_BYPASS_TOKEN` and `CRON_SECRET`, while this repo's workflows reference five. **`db-backup.yml` needs `BACKUP_DATABASE_URL` and `BACKUP_PASSPHRASE`, and the existing doctor has never looked at either** — so a backup failing on an unset passphrase was invisible to the tool built to see exactly that. That workflow deliberately fails loudly rather than skipping ("a backup job that reports success without producing a backup is worse than no backup workflow at all"), which makes the gap worse, not milder: the run goes red and nothing explains why.

`VERCEL_BYPASS_TOKEN` is declared optional in `.github/floor.json`, carrying over `tools/doctor/check.js`'s own `required: false` judgement rather than re-litigating it.

`tools/doctor/` and `doctor.yml` are **deliberately left in place**. The generic check is a strict superset of what they do, but they are referenced by seven files here — including `tests/doctor.test.js`, `tools/coverage-gaps/check.js`, `tools/db-health/probes.js` and this document — so removing them is its own change with its own blast radius, not a side effect of this one. Doing it would also need `tools/doc-currency` re-run, since docs cite the file paths.

The `drift` job runs on every pull request and touches no secrets context; the `probe` job runs on dispatch only. Regenerate the env block with `node scripts/secrets-doctor.mjs --sync`.
