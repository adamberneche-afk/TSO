...
>
> **Update — 2026-09-12 (session wind-down):** Closing out this session.
> Since the 2026-09-08 follow-up above, four PRs merged to `main`: audit
> submission/trust-score computation and real CLI-and-SDK HTTP calls
> (rows 6, 7, 9 -- from that 2026-09-08 update itself), then `#2026`
> (row 6's real multi-party provenance chain), `#2027` (row 8's sandbox
> decision recorded as permanent; row 14's App RAG built, Enterprise RAG
> data model designed), and `#2028` (row 22's skill-publishing flow
> built, Agent Marketplace data model designed). `docs/
> DOCS_VS_CODEBASE.md` now stands at **19 BUILT · 3 PARTIAL · 2 NOT
> BUILT** of 24, up from 12 BUILT · 6 PARTIAL · 5 NOT BUILT of 23 at the
> start of the 2026-09-07 remediation pass (the catalogue itself grew by
> one row -- #24, usage analytics -- along the way). A final
> documentation-accuracy pass then found and fixed four stale spots
> elsewhere in the repo: `tais_frontend`'s `README.md`/`IMPLEMENTATION.md`
> roadmap checklists still showed "skill publishing" as unbuilt;
> `packages/agent-sdk/README.md` was missing the `rag:read` scope and its
> `getRagDocuments()` method from its docs entirely; the RAG README's
> version footer was stale relative to its own body text. That fix is
> commit `7de09bd` on this session's branch -- **not yet in its own PR**,
> deliberately held to fold into whichever PR a next session opens first
> rather than shipped standalone. See "Where things stand" below for
> exact current branch/PR state.
...
**As of the original 2026-09-06 session** (kept as that session's own
point-in-time record): 1 open issue (`#2010`), 7 open PRs (the risky
Dependabot bumps), `main` at commit `0bf1a74`.

**As of this session's 2026-09-12 wind-down** (current):

- **Open issues**: still 1 (`#2010`, the watchdog — legitimate, leave it).
- **Open PRs**: 11 Dependabot bumps, none evaluated this session: 4 of
  the 7 flagged as risky in the 2026-09-06 review above are still open
  (`#2008` chalk, `#2006` @sentry/node, `#2000` rand, `#1993` vite — the
  other 3 from that list, `#2007`/`#1997`/`#1996`, are no longer open),
  plus 7 more that have appeared since (`#2013` electron, `#2014`
  @radix-ui/react-label, `#2015` tailwind-merge, `#2016` typescript,
  `#2017` @radix-ui/react-dialog, `#2018` @radix-ui/react-progress,
  `#2019` dotenv). None of these 7 newer ones have been reviewed for
  breaking changes the way the original 7 were — treat the whole list as
  a starting point to re-verify, not gospel.
- **`main`** includes this session's merged PRs — `#2026`, `#2027`, and
  `#2028` (the last, most recent one) — as of commit `b0c21ab`.
- **This session's branch** (`claude/tso-handoff-review-cy2tlh`) is one
  commit ahead of `main`: `7de09bd`, the documentation-accuracy fixes
  described in the wind-down update above. **Not yet in a PR.** A fresh
  session should open a PR for it (or fold it into the first PR of new
  work, per this session's own preference) before starting anything
  else, so it doesn't get lost or silently rebased away.
...
6. `docs/ENTERPRISE_RAG_DATA_MODEL.md` and `docs/
   AGENT_MARKETPLACE_DATA_MODEL.md` — real, migrated, tested schemas with
   no routes/UI yet (rows 14 and 22's PARTIAL halves); read either before
   building on top of it, since each documents an open design question
   (encryption/access-boundary; moderation) deliberately left unanswered
